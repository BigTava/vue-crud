...
describe("AppContent.vue Test with Successful HTTP calls", () => {
  ...
  it("loads the user data when the component is created and mounted", () => {
    ...
    expect(mock.history.get[0].url).toMatch(
    ...
    expect(bannerStore.setBannerData).toHaveBeenLastCalledWith(
      "SUCCESS! Loaded user data!",
      "Success"
    );
  });
  ...
  it("saves the new user data", async () => {
    ...
    expect(mock.history.post[0].url).toMatch(
      "https://jsonplaceholder.typicode.com/users"
    );
  ...
  it("updates the data for user #1", async () => {
    ...
    expect(mock.history.put[0].url).toMatch(
